...
>
> **実装状況**: 型・JSON 往復・v2→v3 移行・エクスプローラ・パレット／ビットマップ編集・
> **キャラ／シーンの Blockly 本接続**・新規時のマッパー選択ウィザード・
> **Create ビルドは Mapper 0/1/2/3/4/7/30（単一バンク相当）**・複数シーン `gotoScene`・
> BGM ループ・背景 `fillBackground`/`setScroll`・`drawSpriteFlip` まで。
> バンク切替／IRQ／MapAsset 本実装は後続。

## マッパーと命令の関係（現状）

- **標準ブロック／DSL API はマッパー共通**（描画・音・入力・パーツ／シーン・背景スクロール）。
- 変わるのは **容量の目安**。`.nes` 出力は上記マッパーでビルド可（PRG は 16KB 生成＋拡張複製）。
- 非 NROM ではツールボックスに「バンク（準備中）」カテゴリを出し、差を見える化する。
- バンク切替・IRQ 専用ブロックは未実装（エミュ再生側の Mapper 対応とは別レイヤ）。
...
- [x] ツリーからシーン／画像／キャラ／音に辿って編集できる
- [x] 紐づけがピッカー／ID 参照で、手書き JS が必須でない（ブロック主）
- [x] ブロックが主編集面（コードはビルド中間）
- [x] 8×8 以外のビットマップ（アップロード含む）を扱える
- [x] 新規プロジェクトでマッパーを選ぶ
- [x] v3 から `.nes` がビルドできる（**Mapper 0 のみ**）
- [ ] 他マッパーのバンク命令／容量チェック付きビルド
